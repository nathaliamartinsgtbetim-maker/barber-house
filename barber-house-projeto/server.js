...
app.post('/api/clientes', async (req, res) => {
  try {
    ...
    if (!nome || !telefone) return res.status(400).json({ erro: 'Preencha nome e telefone.' });

    const [existentes] = await pool.query('SELECT * FROM clientes WHERE telefone = ?', [telefone]);
    if (existentes.length) {
      await pool.query('UPDATE clientes SET nome = ? WHERE id = ?', [nome, existentes[0].id]);
      return res.json({ id: existentes[0].id, nome, telefone });
    }
    const [r] = await pool.query('INSERT INTO clientes (nome, telefone) VALUES (?,?)', [nome, telefone]);
